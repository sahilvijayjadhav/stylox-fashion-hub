        		...
        		<table class="table ">
        			...
        			<%
        				...
        				if (searchName != null && !searchName.isEmpty()) {
                            sql = "SELECT * FROM b_products WHERE b_pName LIKE ?";
        				...
        				rs = ps.executeQuery();
        				while(rs.next()){
        					String colors = rs.getString("b_colors");
        					String[] colorArray = colors.split(",");
        					String size = rs.getString("b_sizes");
        					String[] sizeArray = size.split(",");
        					String imageNames = rs.getString("b_pImg");
        			...
        			%>
        				<tr>
        					<th scop="row"><%=rs.getInt("pId") %></th>
        					<td>
        						<img alt="<%=image %>" class="" width="100px" src="./images/illustration/<%=rs.getString("b_catogary") %>/<%=image %>">
        					</td>
        					<td><%=rs.getString("b_pName") %></td>
        					<td>&#8377; <%=rs.getString("b_prize") %></td>
        					<td><%=rs.getString("b_quantity") %></td>
        					<td><%=rs.getString("b_catogary") %></td>
        					<td>
        						<button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModal<%=rs.getInt("pId") %>">Colors & Sizes</button>
        						<div class="modal fade" id="exampleModal<%=rs.getInt("pId") %>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
